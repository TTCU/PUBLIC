  ...
  (function () {
    ...
    function ttcuCssText() {
      ...
      return ""
        ...
        + "#ttcuChatLauncher{position:fixed;" + side + ":var(--ttcu-gap);bottom:var(--ttcu-gap);width:56px;height:56px;border:0;border-radius:50%;background:#0f6
  cf2;color:#fff;box-shadow:var(--ttcu-chat-shadow);cursor:pointer;z-index:" + (OPT.zIndex - 1) +
  ";display:flex;align-items:center;justify-content:center;font-size:22px}"
        + "#ttcuChatPanel{position:fixed;" + side + ":var(--ttcu-gap);bottom:calc(var(--ttcu-gap) +
  64px);width:var(--ttcu-chat-width);height:var(--ttcu-chat-height);max-width:min(95vw," + OPT.maxWidth + "px);max-height:min(80vh," + OPT.maxHeight +
  "px);background:#fff;border:var(--ttcu-chat-border);border-radius:var(--ttcu-chat-radius);box-shadow:var(--ttcu-chat-shadow);overflow:hidden;transform:translat
  eY(16px);opacity:0;pointer-events:none;transition:transform .2s ease,opacity .2s ease;z-index:" + OPT.zIndex + "}"
        + "#ttcuChatPanel.open{transform:translateY(0);opacity:1;pointer-events:auto}"
        + "#ttcuChatHeader{height:var(--ttcu-header-h);display:flex;align-items:center;justify-content:space-between;padding:0 10px 0 12px;border-bottom:1px
  solid rgba(0,0,0,.06);background:linear-gradient(0deg,#fff,#fafafa);font:600 14px/1.2 system-ui,-apple-system,Segoe UI,Roboto,Arial;color:#1f2937}"
        ...
        + "@media (max-width:640px){#ttcuChatLauncher{" + side + ":max(var(--ttcu-gap),env(safe-area-inset-" + side +
  "));bottom:max(var(--ttcu-gap),env(safe-area-inset-bottom))}"
    ...
    function ttcuBuild() {
      ...
      if (OPT.launcher && !ttcuById("ttcuChatLauncher")) {
        ...
        launcher.innerHTML = '<svg width="26" height="26" viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M20 2H4a2 2 0 0 0-2 2v18l4-4h14a2 2
   0 0 0 2-2V4a2 2 0 0 0-2-2Z"/></svg>';
      ...
      if (!ttcuById("ttcuChatPanel")) {
        ...
        header.innerHTML = '<div id="ttcuChatTitle"></div><div class="actions"><button id="ttcuChatMin" title="Minimize" aria-label="Minimize">▾</button><button
  id="ttcuChatClose" title="Close" aria-label="Close">✕</button></div>';
    ...
    // GoalDigger Mic Drop Handler - MOVED INSIDE THE IIFE
    window.addEventListener('message', function(event) {
      if (event.data && event.data.type === 'GOALDIGGER_MIC_DROP') {
        if (event.data.action === 'show') {
          // Create full-screen mic drop overlay
          var overlay = document.createElement('div');
          overlay.innerHTML = event.data.html;
          var micDropElement = overlay.firstElementChild;
          micDropElement.id = 'goaldigger-fullscreen-micdrop';

          // Make it full screen with highest z-index
          micDropElement.style.position = 'fixed';
          micDropElement.style.top = '0';
          micDropElement.style.left = '0';
          micDropElement.style.width = '100vw';
          micDropElement.style.height = '100vh';
          micDropElement.style.zIndex = (OPT.zIndex + 1).toString(); // Higher than chat

          document.body.appendChild(micDropElement);

          // Add click and ESC listeners
          var closeMicDrop = function() {
            var overlay = document.getElementById('goaldigger-fullscreen-micdrop');
            if (overlay) {
          ...
          micDropElement.addEventListener('click', closeMicDrop);
